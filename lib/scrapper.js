...
const WikiScrapper = {
    ...
    GetOnThisDay: function (params, callback) {
        ...
        var category = params.categories ?? ["events", "births", "deaths"], month, day

        try {
            month =  monthNames[parseInt(params.month) - 1] || monthNames[date.getMonth()];
            day = params.day ?? date.getDay();
        } catch{
        ...
        this.GetHtml(endpoint, (response)=>{
            ...
            for (var i = 0; i < category.length; i++) {
                ...
                var rArray = []
                $(id).parent().nextUntil("h2").each(function () {
                    var x = $(this);
                    if (x.is("ul")) {
                        var li = $(this).find("li")
